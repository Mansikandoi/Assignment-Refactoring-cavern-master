...
def block(x,y):
    ...
    if grid_y > 0 and grid_y < NUM_ROWS:
        row = app.screen.game.grid[grid_y]
...
class Orb(CollideActor):
    ...
    def update(self):
        ...
        if self.timer == self.blown_frames:
            ...
            self.game.pops.append(Pop(self.pos, 1))

            if self.trapped_enemy_type is not None:
                self.game.fruits.append(
                    Fruit(self.game, self.pos, self.trapped_enemy_type)
                )
...
class Bolt(CollideActor):
    ...
    def update(self):
        ...
        idx = "1" if self.direction_x > 0 else "0"
        frame = str((self.game.timer // 4) % 2)

        self.image = "bolt" + idx + frame
...
class Fruit(GravityActor):
    ...
    def update(self):
        ...
        if self.time_to_live <= 0:
            self.game.pops.append(Pop((self.x, self.y - 27), 0))

        frame = str([0, 1, 2, 1][(self.game.timer // 6) % 4])

        self.image = "fruit" + str(self.type) + frame
...
class Game:
    ...
    def update(self, input_state):
        ...
        # Update player separately (needs input_state)
        if self.player:
        ...
        if self.timer % 100 == 0 and len(self.pending_enemies + self.enemies) > 0:
            ...
            self.fruits.append(
                Fruit(self, (randint(70, 730), randint(75, 400)))
            )
...
class MenuScreen:
    ...
    def draw(self):
        ...
        frame = min(((self.game.timer + 40) % 160) // 4, 9)

        screen.blit("space" + str(frame), (130, 280))
...
class PlayScreen:
    ...
    def update(self, input_state):
        ...
        # Check for pause FIRST (before any other logic)
...
class PauseScreen:
    """Pause overlay - freezes game simulation"""
    
    def __init__(self, app, play_screen):
        self.app = app
        self.play_screen = play_screen  # Keep reference to resume

    def update(self, input_state):
        # Check for unpause (P pressed again)
        if input_state.pause_pressed:
            self.app.change_screen(self.play_screen)  # Resume
...
def update():
    ...
    state = input_manager.build(keyboard)
